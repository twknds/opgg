		...
		<c:forEach var="leagueInfo" items="${leagueInfo}" varStatus="s" >
			<div class ="row h-100 justify-content-center align-items-center">
				...
				<div class ="col-md-6" style="text-align: center;">
					<c:choose>
						<c:when test="${leagueInfo.getQueueType() == 'RANKED_FLEX_SR'}"> <h2>자유 랭크</h2> </c:when>
						<c:when test="${leagueInfo.getQueueType() ==  'RANKED_TFT'}"> <h2>전략적 팀전투</h2></c:when>
						<c:when test="${leagueInfo.getQueueType()== 'RANKED_SOLO_5x5'}"> <h2>솔로 랭크</h2></c:when>
					</c:choose>
					<h3>승리/패배 </h3>
